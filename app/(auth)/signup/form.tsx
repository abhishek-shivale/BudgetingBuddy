...
const initialState = { loading: false, email: '', password: '', showPassword: false, success: false, error: '' }
...
function Form() {
  ...
  const HandelSubmit = async(e: React.FormEvent) => {
    ...
    if(!state.email || !state.password) {
      return setState((prev) => ({ ...prev, error: 'Email and Password are required', success: false }));
    }
    setState((prev) => ({ ...prev, loading: true, error: '', success: false }));
    try {
     const {data: response} = await instance.post(apiUrl.auth.signup, {
        email: state.email,
        password: state.password
      })
      if(response.success) {
        setState({ ...state, loading: false, success: true, error: response.message })
        router.push('/dashboard')
      }

      if(!response.success) {
        setState({ ...state, loading: false, success: false, error: response.message })
      }
    } catch (error: any) {
    ...
    <form onSubmit={HandelSubmit}>
      ...
      <label htmlFor="password" className="mb-1 block">
        ...
        <div className="relative">
          ...
          >
            {state.showPassword ? (
              <EyeOff onClick={() => setState({ ...state, showPassword: false })} size={16} />
            ) : (
              <Eye onClick={() => setState({ ...state, showPassword: true })} size={16} />
            )}
          </button>
        </div>
      </label>
      <Button
        variant="default"
        className="mt-6 w-full bg-gray-800 hover:bg-gray-900 relative"
      >
      ...
      <p
			...
			>
				{state.success && !state.error ? (
					<span className="text-green-700">Signup Successful </span>
				) : null}
